...
def parse_arguments():
    ...
    parser.add_argument('--mode', default='very-sensitive', choices=[])
    parser.add_argument('--threads', default = 5)
    parser.add_argument('--prefix', default='_bowtie2Mapped', type=str)
    parser.add_argument('--no_discordant', default = False)
    parser.add_argument('--sample_name', required=True)
    parser.add_argument('--pair_ends', action ='store_false')
    parser.add_argument('--preserve_sam', default = True)
...
def get_align_qc_report(input_bam, prefix, sample_name):
    ...
    with open(align_qc_report, 'a') as f:
        print(f"{'*' * 20} {input_bam} full mapping QC Report by samtools stat {'*' * 20}", file = f)
    command = f'samtools stat {input_bam} |grep ^SN | cut -f 2- >> {align_qc_report}'
    subprocess.run(command, shell=True)
    with open(align_qc_report, 'a') as f:
        print('\n\n', file = f)
        print(f"{'*' * 20} {input_bam} full mapping QC Report by samtools flagstat {'*' * 20}", file = f)
...
# align
    ...
    full_align_qc_report = get_align_qc_report(input_bam=rmdup_bam, prefix=args.prefix, sample_name=args.sample_name)
# rmdup
